...
class InkuireDocumentableToPageTranslator(val renderingStrategy: (callback: (DriResolver?, DokkaConfiguration.DokkaSourceSet) -> String, sourceSet: DokkaConfiguration.DokkaSourceSet) -> RenderingStrategy) {
    ...
    fun invoke(module: DModule): List<PageNode> = module.packages.let { packages ->
        ...
        module.sourceSets.flatMap { sourceSet ->
            val callback: (DriResolver?, DokkaConfiguration.DokkaSourceSet) -> String = { locationResolver, sourceSet ->
                functions.filter {
                    sourceSet in it.sourceSets && sourceSet.dependentSourceSets.none { d -> d in it.sourceSets.map { it.sourceSetID } } && it.isNotOverrideOrInherited(sourceSet)
